          ...
          <div className="space-y-10 animate-in slide-in-from-right-4">
             <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest italic">Xử lý lỗi kiểu dữ liệu (integer vs object)</p>
                </div>
             </div>
             ...
             <div className="glass-card p-10 rounded-[3rem] border border-red-500/20 bg-red-500/5 space-y-6">
                <div className="flex items-center gap-3">
                   ...
                   <h4 className="text-lg font-black text-white uppercase italic">FIX LỖI KIỂU DỮ LIỆU "{}"</h4>
                </div>
                <p className="text-slate-400 text-sm font-medium italic">Lỗi của bạn xảy ra vì cột <b>task_counts</b> hoặc các cột số đang để sai kiểu dữ liệu. Hãy chạy mã SQL này để chuyển sang <b>JSONB</b> và <b>NUMERIC</b>.</p>
...
NOTIFY pgrst, 'reload schema';`}
                   ...
                   <button onClick={copySql} className="absolute top-4 right-4 p-4 bg-slate-900/80 border border-white/5 rounded-xl text-slate-400 hover:text-white transition-all active:scale-90 shadow-2xl">
        ...
        {tab === 'users' && (
           <div className="space-y-10 animate-in slide-in-from-right-4">
             <div className="relative">
                ...
                <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Tìm hội viên..." className="w-full bg-slate-900 border border-slate-800 rounded-3xl pl-20 pr-10 py-6 text-white font-bold outline-none focus:border-blue-500 shadow-inner" />
             </div>
                 ...
                 {allUsers.filter(u => u.fullname.toLowerCase().includes(searchTerm.toLowerCase())).map(u => (
                   <tr key={u.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                     ...
                     <td className="px-10 py-6 text-right">
                        <button onClick={() => refreshData()} className="p-3 text-blue-500 hover:scale-110 transition-transform"><Activity className="w-5 h-5" /></button>
                     </td>
                   </tr>
                 ))}
               </tbody>
